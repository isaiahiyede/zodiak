...
def add_job4(request,jobtype):
    context = {}
    print(request.POST)
    shpps = [shipper.shippers_name for shipper in Shippers.objects.filter(deleted=False) ]
    context['shippers'] = shpps
...
def add_job(request,jobtype):
    ...
    print(request.POST)
    shpps = [shipper.shippers_name for shipper in Shippers.objects.filter(deleted=False) ]
    context['shippers'] =json.dumps(shpps)
    if request.method == "POST":
        form = JobForm(request.POST)
        if form.is_valid():
            get_shipper = request.POST.get('shippers_name')
            if Shippers.objects.filter(shippers_name=get_shipper).exists():
                pass
            else:
                shp_obj = Shippers.objects.create(
                    shippers_name = request.POST.get('shippers_name'),
                    shippers_email = request.POST.get('shippers_email'),
                    shippers_number = request.POST.get('shippers_number'),
                    shippers_country = request.POST.get('shippers_country'),
                    shippers_address = request.POST.get('shippers_address')
                    )
